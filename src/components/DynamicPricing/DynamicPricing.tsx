...
export function DynamicPricing({
  ...
  const tier4Rate = baseCastRate * pricing_tiers.tier4;
  const tier5Rate = baseCastRate * pricing_tiers.tier5;
  ...
  const weightedAverageRate = useMemo(() => {
    ...
    const tier4Avg = (tierRates.tier4?.rate || 0) * tier4Rate;
    const tier5Avg = (tierRates.tier5?.rate || 0) * tier5Rate;
    return tier1Avg + tier2Avg + tier3Avg + tier4Avg + tier5Avg;
  }, [tierRates, tier1Rate, tier2Rate, tier3Rate, tier4Rate, tier5Rate]);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-1.5">
          ...
          <div className="bg-orange-500/10 rounded-xl p-2 border border-purple-500/20">
            <div className="text-[8px] text-purple-400/70">Tier 3 (3.5x)</div>
            <div className="text-[11px] font-semibold text-purple-400">
              ${tier4Rate.toFixed(2)}
            </div>
          </div>
          <div className="bg-yellow-500/10 rounded-xl p-2 border border-purple-500/20">
            <div className="text-[8px] text-purple-400/70">Tier 3 (3.5x)</div>
            <div className="text-[11px] font-semibold text-purple-400">
              ${tier5Rate.toFixed(2)}
            </div>
      ...
      <div className={`${panelClasses} p-4 space-y-3`}>
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
          ...
          <Input
            ...
            value={desiredReach}
            onChange={(e) =>
              setDesiredReach(Math.max(1, Number.parseInt(e.target.value) || 1))
            }
      ...
      <div className={`${panelClasses} p-4 space-y-1`}>
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-xl font-bold text-white">
            {(totalBudget * 1.1).toFixed(2)}{" "}
            <span className="text-xs text-purple-400">USDC</span>
          </span>
        </div>
        <p className="text-[9px] text-white/50 text-right mt-0.5">
          ~{desiredReach} casts × ${weightedAverageRate.toFixed(3)} avg + 10%
          protocol fee
        </p>
      </div>
    </div>
