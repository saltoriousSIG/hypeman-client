...
export function AddBudgetModal({
  ...
  const get_allowance = useContract(
  ...
  const { data: allowance, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      setApproved(parseFloat(formatUnits(user_allowance.toString(), 6)) > total);
  ...
  const handleApprove = useCallback(async () => {
    ...
    const tx = await approve([
      DIAMOND_ADDRESS,
      parseUnits(calculated_allowance.toString(), 6),
    ]);
    setApproved(true);
  ...
  const handleConfirm = useCallback(async () => {
    ...
    await handleAddPromotionBudget(
      promotion.id,
      parseUnits(numAmount.toString(), 6)
    );
    setAmount("");
  ...
  const newTotal =
    parseFloat(formatUnits(promotion.total_budget, 6)) +
    (Number.parseInt(amount) || 0);
  const quickAmounts = [50, 100, 250, 500];

  console.log(
    parseFloat(formatUnits(allowance?.toString() || 0n, 6)) > parseFloat(amount)
  );
  console.log(parseFloat(formatUnits(allowance?.toString() || 0n, 6)), parseFloat(amount))
      ...
      <DialogContent className="bg-black/95 backdrop-blur-xl border-emerald-500/20 text-white w-[95%] rounded">
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {(allowance &&
                parseFloat(formatUnits(allowance.toString(), 6)) >
                  parseFloat(amount)) ||
              approved
                ? `Add $${amount || "0"}`
                : `Approve $${amount || "0"}`}
