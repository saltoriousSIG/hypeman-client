  ...
  try {
    for (const log of req.body) {
      const decoded: any = decodeEventLog({
        abi: [
          data.find((x: any) => {
            ...
            return x.name === "PromotionCreated" && x.type === "event";
          })!,
        ],
        data: log.data,
        topics: log.topics,
      ...
      pipeline
        ...
        .sadd(`promotion:${promotion.pro_user}`, promotion.id.toString())
        .sadd(`promotion_state:${promotion.state}`, promotion.id.toString());
      await pipeline.exec();
    }
