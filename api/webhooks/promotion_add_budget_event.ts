...
async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.body.length > 0) {
      console.log(
        "PROMOTION ADD BUDGET EVENT FIRED vvv =============================================================================================== vvv"
      );
      ...
      for (const log of req.body) {
        const decoded: any = decodeEventLog({
          abi: [
            data.find(
              (x: any) => x.name === "BudgetAdded" && x.type === "event"
            )!,
          ],
          data: log.data,
          topics: log.topics,
        ...
        if (decoded && decoded.eventName === "BudgetAdded") {
    ...
    return res.status(200).json({
      message: "Promotion add budget Event Processed",
